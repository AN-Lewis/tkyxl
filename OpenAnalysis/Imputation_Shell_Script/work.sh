...
#plink association testの結果を得る
	...
	do 
		...
		gtool -G --g ./results/gwas_data_koa-${rs_number}.best_guess_haps_imputation.impute2 --s ./data_files/koa-chr${chromosome}.sam --ped ./data_files/koa-${rs_number}.ped --map ./data_files/koa-${rs_number}.map --sex sex --phenotype phenotype --chr ${chromosome}
		plink --noweb --file ./data_files/koa-${rs_number} --assoc --ci 0.95 --adjust --out ./results/koa-${rs_number}.out
	done
#done
